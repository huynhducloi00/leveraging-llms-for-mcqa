...
class ModelResponseNatural:
    logprobs: dict # top 200 most probable logits
    answer_logits: dict # logit of only the answer tokens
    response_list: list
...
class Test:
    ...
    def process_question_natural(self, question):
        ...
        return ModelResponseNatural(logprobs=logprobs, response_list=list())
...
class GPT2Model:
    ...
    def process_question_natural(self, question):
        ...
        answer_logits_dict = {
            k: v
            for k, v in logprobs_dict.items() if k in GPT2Model.MAGIC_ARRAY
        }
        # Reduce logprobs_dict to only keys with top 50 largest values
        logprobs_dict = {
            k: v
            for k, v in sorted(
                logprobs_dict.items(), key=lambda item: item[1], reverse=True
            )[:200]
        }

        return ModelResponseNatural(logprobs=logprobs_dict,answer_logits=answer_logits_dict, response_list=list())
